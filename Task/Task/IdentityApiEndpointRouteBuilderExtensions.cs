...
public static class IdentityApiEndpointRouteBuilderExtensions
{
    ...
    private record CustomRegisterRequest(string Email, string Password, string FirstName, string LastName, UserRoles Role, string? LeadEmail);
    
    /// <summary>
    /// Add endpoints for registering, logging in, and logging out using ASP.NET Core Identity.
    /// </summary>
    /// <typeparam name="AppUser">The type describing the user. This should match the generic parameter in <see cref="UserManager{AppUser}"/>.</typeparam>
    ...
    public static IEndpointConventionBuilder MapCustomIdentityApi(this IEndpointRouteBuilder endpoints)
    {
        ...
        // NOTE: We cannot inject UserManager<AppUser> directly because the AppUser generic parameter is currently unsupported by RDG.
            ...
            ([FromBody] CustomRegisterRequest registration, HttpContext context, [FromServices] IServiceProvider sp) =>
        {
            ...
            AppUser? lead = null;
            if (registration.LeadEmail is not null)
            {
                lead = await userManager.FindByEmailAsync(registration.LeadEmail);
            }
            
            var user = new AppUser()
            {
                LeadId = registration.Role == UserRoles.Normal? lead?.Id : null
            };
            ...
            var claimsList = new List<Claim>
            {
                ...
                new Claim(ClaimTypes.GivenName, $"{registration.FirstName} {registration.LastName}"),
                new Claim(ClaimTypes.Role, registration.Role.ToString()),
            };
            
            var addingClaimsResult = await userManager.AddClaimsAsync(user, claimsList);

            if (!addingClaimsResult.Succeeded)
            {
                return CreateValidationProblem(addingClaimsResult);
            }
            ...
            ([FromBody] RefreshRequest refreshRequest, [FromServices] IServiceProvider sp) =>
        {
            ...
            if (refreshTicket?.Properties?.ExpiresUtc is not { } expiresUtc ||
                ...
                await signInManager.ValidateSecurityStampAsync(refreshTicket.Principal) is not AppUser user)
        ...
        async Task SendConfirmationEmailAsync(AppUser user, UserManager<AppUser> userManager, HttpContext context, string email, bool isChange = false)
        {
    ...
    private static async Task<InfoResponse> CreateInfoResponseAsync<AppUser>(AppUser user, UserManager<AppUser> userManager)
        where AppUser : class
    {
        return new()
        {
